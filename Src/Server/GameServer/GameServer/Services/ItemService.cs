...
namespace GameServer.Services
{
    internal class ItemService : Singleton<ItemService>
    {
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnItemEquip: Character:{0}; Slot:{1}; Item:{2}; Equip:{3}}", character.Id, request.Solt, request.itemId,request.isEquip);
            var result = EquipManager.Instance.EquipItem(sender,request.Solt,request.itemId,request.isEquip);
